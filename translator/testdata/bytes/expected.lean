...
def addBytes (bs : List F) : Circuit F := do
  let mut acc := (0 : F)
  for b in bs do
    acc := Gates.add acc b
  return acc

def circuit (Sum : F) : Circuit Unit := do
  let dom := [(97 : F), (98 : F), (99 : F)]
  let mut vars := List.replicate dom.length (0 : F)
  for i in goRange 0 (Int64.ofNat dom.length) do
    let b := dom[i.toInt.toNat]!
    vars := vars.set i.toInt.toNat b
  let t_0 ← addBytes vars
  Gates.eq t_0 Sum
