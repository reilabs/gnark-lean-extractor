...
structure DoubleGadget where
  X : F
  deriving Inhabited

def MixIn (a : F) (b : F) : Circuit F := do
  let s := Gates.add a b
  return Gates.mul s s

def Pair.Combine (p : Pair) : Circuit F := do
  return Gates.add p.Lo p.Hi

def DoubleGadget.DefineGadget (g : DoubleGadget) : Circuit F := do
  return Gates.mul g.X (2 : F)

def circuit (A : F) (B : F) (R : F) : Circuit Unit := do
  let m ← MixIn A B
  let p := ({ Lo := A, Hi := B : Pair })
  let q ← p.Combine
  let d ← ({ X := A : DoubleGadget }).DefineGadget
  let mut sum := Gates.add m q
  sum := Gates.add sum d
  Gates.eq sum R
