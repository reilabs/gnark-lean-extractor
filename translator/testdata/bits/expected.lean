...
def circuit (In : F) (Sum : F) (N : Int64) : Circuit Unit := do
  let bits ← Gates.toBinary In 8
  let mut acc := (0 : F)
  for b in bits do
    Gates.isBool b
    acc := Gates.add acc b
  for i in goRange 0 N do
    let t_0 ← Circuit.get bits i.toInt.toNat
    Gates.isBool t_0
  Gates.eq acc Sum
